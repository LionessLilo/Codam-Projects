...
/*   Updated: 2025/08/18 11:22:18 by lilo          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	test_input_checks(void)
{
	...
	log_test(result == -1, "Input checks is not returning an error for not enough args");

	char 	*test3[] = {"name", "10", "2147483649", "200", "200", NULL};
	result = input_checks(5, test3);
	log_test(result == -1, "Input checks is not returning an error for greater than MAX INT");

	char 	*test4[] = {"name", "10", "-2147483649", "200", "200", NULL};
	result = input_checks(5, test4);
	log_test(result == -1, "Input checks is not returning an error for not less than MIN INT");

	char 	*test5[] = {"name", "600", "200", "200", "200", NULL};
	result = input_checks(5, test5);
	log_test(result == -1, "Input checks is not returning an error for greater than 500 philosophers");

	char 	*test6[] = {NULL};
	result = input_checks(0, test6);
	log_test(result == -1, "Input checks is not returning an error for NULL input");
	...
	log_test(result == -1, "Input checks is not returning an error for argc 2 and more than 200 philos");
